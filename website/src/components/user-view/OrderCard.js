...
const OrderCard = props => {
    ...
    return (
        ...
        <Card className="order-card-main">
            <i class="far fa-times-circle" style={{ position: 'relative', left: '-10px', textAlign: 'left', width: '100%', marginTop: '3%', fontSize: '1.5rem' }} onClick={() => props.closeOrderCard()}></i>
                ...
                {props.order.map(item => (
                    <div key={item.item} className="order-item-div">
                        <div className="order-items-cont-txt order-item-remove">
                            <i class="fas fa-times-circle order-items-cont-txt" style={{ width: '10%'}} onClick={() => props.removeFromOrder(item.item)}></i>
                        </div>
                        ...
                        <p className="order-item-name order-items-cont-txt">{item.item}</p>
                        <p className="order-item-total order-items-cont-txt">{CurrencyFormatter.format(item.total, { currency: 'USD' })}</p>
                        <hr />
                    </div>
                ))}
            </div>
